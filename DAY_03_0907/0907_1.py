"""
타이타닉 데이터셋 기초 탐색
pandas head/tail/shape/info/columns를 사용해서 데이터셋의 기본 정보를 
...
# titanic.csv 가져오기
csv_path = "Titanic.csv" #"../comTitanic.csv?"

# st.file_uploader로 파일을 직접 올릴 수 있다, 아무것도 올리지 않으면
# 아래의 기존 로직대로 같은 폴더 타이타닉 파일을 그대로 찾아서 읽는다. 

upload_file = st.file_uploader("Titanic.csv 파일을 직접 업로드 할 수 있습니다", type='csv')
...
if df is not None:
    ...
    # os.StringIO() 라는 "메모리 위에 가짜 파일"을 받아낸 뒤 그 내용을 text로 보여준다.
